class WelcomeController < ApplicationController
  protect_from_forgery :only => [:create, :update, :destroy] #j'ai mis cela pour éviter un bug
	...
	def set_class_object
	   if params[:type]=="user"
		  @classType = User
			@user = true  #retrocompatibility with views
			@tag = false  #
		elsif params[:type]=="tag"
		  @classType = Tag
			@tag = true   #
			@user = false #retrocompatibility with views
		else
